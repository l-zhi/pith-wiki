...
type CodexToolResult = { content?: Array<{ type?: string; text?: string }>; structured_content?: unknown } | null;
...
export async function parseCodexStream(
  ...
  for await (const line of lines) {
    ...
    if (type === 'error' || type === 'stream_error' || type === 'turn.aborted' || type === 'turn.failed') {
      isError = true;
      errorMessage =
        evt.message ??
...
export class CodexAgent implements AgentLike {
  private history: Msg[] = [];
