...
llm-wiki is a solo-maintained open source project. The threat model is real
...
1. **GitHub Security Advisory** (preferred) — go to
   <https://github.com/l-zhi/llm-wiki/security/advisories/new>. This gives
   us a private threaded conversation, optional CVE assignment, and a
   coordinated-disclosure timer.
2. **Email backup** — `lizhi.fan+llm-wiki.security@gmail.com`. Use this only
...
- **API key / secret leakage** — keys ending up in `output/<sessionTs>.md`
  transcripts, `~/.llm-wiki/history`, `<wikiRoot>/.queue/logs/*.log`, or
...
- CVEs in `node_modules` dependencies that are **not reachable** from
  llm-wiki code. (Reachable ones — yes, please report.)
...
- The REPL has a path-level write approval prompt (`y` / `a` / `n`) for
  ...
  scripting.
- API keys are loaded from `~/.llm-wiki/.env` (recommended) or the
