...
const params = z.object({
  path: z
    .string()
    .describe(
      'Target path, relative to the wiki output dir (<wikiRoot>/output). Writes are confined there — you cannot write to the current working directory or elsewhere.',
    ),
...
export const writeFileTool: ToolDef<typeof params> = {
  name: 'write_file',
  description:
    'Write a UTF-8 text file into the wiki output directory (<wikiRoot>/output). Paths are relative to that dir and confined to it; the user must approve the write.',
  parameters: params,
  handler: async ({ path: inputPath, content }, ctx) => {
    let safe: string;
    try {
      // 写入硬收敛到 <wikiRoot>/output：相对路径相对它，绝对路径必须落在它内。
      // 杜绝 agent 把生成文件写到用户运行 pith-wiki 的当前目录。
