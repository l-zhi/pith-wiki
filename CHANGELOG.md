...
> **历史注**：项目原名 `llm-wiki`，于 v0.3.0 改名为 `pith-wiki`。本文件里的全部
> 包名 / 命令名 / 配置路径都已正规化成新名；v0.1.0 段记录的还是当时未发布的内部
> 状态（没有 npm release，只是 git tag），重写为新名不影响历史准确性。
...
- **`additionalReadPaths`**：让 LLM 读项目外的笔记 / 参考资料目录（笔记库、paper
  ...
  `PITH_WIKI_READ_PATHS`（JSON 数组或分隔符串）、`config.json`。
...
- **`wikiRoot` 默认值挪到 `~/.pith-wiki/wiki-data/`**。旧默认是
  `<workspaceRoot>/wiki-data/`，容易被无意提交进项目仓库且多 workspace 无法共享。
  新默认让所有本地数据集中在 `~/.pith-wiki/`。
- **`.env` 默认从 `~/.pith-wiki/.env` 读取**（覆盖项目根 `.env`）。`DEEPSEEK_API_KEY`
...
- **`/digest` 把对话当单边材料压缩**：用户视角丢失（"成长**和低谷期**" → 笼统"成长经历"）。
  ...
  保留对话顺序。
- **ingest 源文件路径未做沙箱校验**：`pith-wiki ingest --file /etc/passwd` 之前
...
- **测试隔离**：`tests/config.test.ts` 不再读维护者本机 `~/.pith-wiki/config.json`；
  通过 `PITH_WIKI_CONFIG_PATH` env 把整个文件级 isolation 锁死。
...
- **新增 [docs/config.example.json](docs/config.example.json)**：完整多 provider
  示例 + watchDirs + queue 配置，可直接 copy 到 `~/.pith-wiki/config.json`。
...
- **库模式 / npm package 入口**（详见 ADR-0001）。`src/index.ts` + barrel exports +
  ...
  未来真有需求时拆 `@pith-wiki/core` + `@pith-wiki/cli`。
...
- **Ink 富终端 REPL**：`App` / `ChatView` / `InputBox` / `ToolApproval` / `TokenMeter`
  ...
  `/reset` `/exit`，对话历史持久化到 `~/.pith-wiki/history`。
...
- **分层配置**：CLI flag > env > `~/.pith-wiki/config.json` > 默认，zod 校验，
  启动时 fail-fast。
...
[0.3.0]: https://github.com/l-zhi/pith-wiki/compare/v0.1.0...v0.3.0
[0.1.0]: https://github.com/l-zhi/pith-wiki/releases/tag/v0.1.0
