# PRD: pith-wiki — Karpathy-style LLM Wiki CLI
...
- **入口**：在终端打 `pith-wiki` 进入富 REPL；也可以用 `pith-wiki ingest|get|list|query` 子命令脚本化。
...
39. As a developer, I want to ingest from stdin pipes, so that I can chain `curl ... | pith-wiki ingest`.
...
- **配置优先级**：CLI flag > env > `~/.pith-wiki/config.json` > 内置默认。zod 校验合并后的对象，启动时 fail-fast。
...
- `pith-wiki`（默认）→ Ink REPL。
- `pith-wiki ingest --collection <c> [--file|--url|-] [--no-auto-link]`。
- `pith-wiki get <id> [--collection <c>]`。
- `pith-wiki list [--collection <c>]`。
- `pith-wiki query <text> [--max-tokens <n>]`。
...
- **冒烟测试**：`npm run build && node dist/bin/pith-wiki.js --help` 手动跑一次确认入口可执行；这一项不进 vitest 因为属于打包级验证。
...
- **PDF / URL 自动抓取** —— `--url` 仅作为 source 标记，不发起 HTTP 请求；用户需自行 `curl | pith-wiki ingest`。
...
- **`pith-wiki doctor` 诊断子命令** —— v0.2 计划。
