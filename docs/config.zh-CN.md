...
| `providers` | _（无 env，复杂结构）_ | `{}`（详见 [usage.zh-CN.md#多-provider-切换](usage.zh-CN.md#多-provider-切换)） |
...
| `watchDirs` | _（无 env）_ | `[]`（详见 [usage.zh-CN.md#目录监听-watcher](usage.zh-CN.md#目录监听-watcher)） |
...
| `PITH_WIKI_HOME` | `PITH_WIKI_HOME` | `~/.pith-wiki/`——覆盖整个 home 目录位置（dev/prod 隔离用） |
...
>
> **想让 dev 和 prod 用两份独立数据？** 设 `PITH_WIKI_HOME=~/.pith-wiki-dev`
> 把 config、queue、wiki-data、transcripts 全套挪走。仓库里的
> `bin/pith-wiki-dev` shim 帮你自动设这条 env。
